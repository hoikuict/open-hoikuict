...
def pickup_revision(record: AttendanceRecord | None) -> str:
    ...
    values = (record.id, ensure_utc(record.updated_at).isoformat(), record.planned_pickup_time,
              record.pickup_person, record.snack_required, record.pickup_snack_confirmed,
              str(record.check_in_at), str(record.check_out_at))
...
def save_pickup_plan(session: Session, *, child_id: int, day, revision: str, planned_pickup_time: str,
                     ...
                     allow_partial: bool = False, snack_confirmed: bool | None = None):
    ...
    if (time or not allow_partial) and not re.fullmatch(r"(?:[01]\d|2[0-3]):[0-5]\d", time):
        raise HTTPException(400, "お迎え予定時刻を時・分で入力してください")
    if (not person and not allow_partial) or len(person) > 100 or any(ord(c) < 32 for c in person):
    ...
    old_confirmed = bool(record and record.pickup_snack_confirmed)
    snack = old_snack if snack_required is None else snack_required
    confirmed = snack_confirmed if snack_confirmed is not None else (True if snack_required is not None else old_confirmed)
    if record is None and allow_partial and not time and not person and not confirmed:
        return None
    if record and (old_time or "", old_person or "", old_snack, old_confirmed) == (time, person, snack, confirmed):
        return record
    now = utc_now()
    values = dict(planned_pickup_time=time or None, pickup_person=person or None,
                  snack_required=snack, pickup_snack_confirmed=confirmed, updated_at=now)
    if record is None:
    ...
    session.add(AttendancePickupHistory(attendance_record_id=record.id, previous_time=old_time,
        previous_person=old_person, previous_snack_required=old_snack if old_confirmed else None, new_time=time, new_person=person,
        new_snack_required=snack if confirmed else None, changed_by_name=actor_name, source=source,
