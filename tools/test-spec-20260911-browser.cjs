...
(async () => {
  ...
  try {
    ...
    for (const width of [390, 768, 1280]) {
      ...
      await page.goto(`${base}/guardian/?class_id=1&child_id=2&date=${today}`);
      ...
      await page.waitForFunction(() => getComputedStyle(document.querySelector('[data-pickup-hour="16"]')).fontSize === '30px');
      ...
      assert(await page.getByText('17:30', {exact:true}).evaluate(el => parseFloat(getComputedStyle(el).fontSize) >= 30));
      await shot(`kiosk-confirm-${width}.png`);
    }
    console.log('PASS: kiosk input and large confirmation at 390/768/1280px');
