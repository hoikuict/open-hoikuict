...
def data_transfer_page(
    ...
    if preview:
        ...
        filename, content, exclusions = _load_preview_file(preview, "families", owner=_preview_owner(current_user), with_options=True)
...
def repreview_families(request: Request, preview_token: str = Form(""), excluded_rows: list[int] = Form(default=[]),
    ...
    _require_dataset_access(current_user, "families")
    filename, content = _load_preview_file(preview_token, "families", owner=_preview_owner(current_user))
    result = preview_import(session, "families", filename, content, excluded_rows=excluded_rows)
    if result.source_rows:
        result.preview_token = _save_preview_file("families", filename, content, owner=_preview_owner(current_user),
...
async def commit_import_file(
    ...
    filename, content, revision, exclusions = _load_preview_file(preview_token, dataset, owner=_preview_owner(current_user), claim=True, with_options=True)
