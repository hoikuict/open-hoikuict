...
def copy_sqlite_snapshot(source_path: Path, db_path: Path) -> None:
    ...
    try:
        ...
        destination.close()
        source.close()
    _upgrade_parent_push_snapshot(db_path)


def _upgrade_parent_push_snapshot(db_path: Path) -> None:
    """Add the current push schema to a packaged demo snapshot before cloning it."""
...
def _apply_parent_push_delivery_columns(target_engine: Engine) -> None:
    with target_engine.connect() as conn:
        columns = [
            row[1]
            for row in conn.execute(text("PRAGMA table_info(parent_notification_deliveries)"))
        ]
        if not columns:
            return
        additions = {
...
def _migrate_parent_push_delivery_columns() -> None:
    try:
        _apply_parent_push_delivery_columns(engine)
    except Exception as exc:
