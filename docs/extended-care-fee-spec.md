...
請求転送済みの料金は、確認済みも含める指定でも再計算しない。先に請求転送側の解除・ロック条件を確認する。

理由付きの誤打刻取消では、未確定料金を削除し、次の降園打刻で再計算する。確認済み・調整済み・対象外・請求転送済みの料金がある打刻は取消を拒否する。通常の再計算と別の操作であり、元の打刻と料金のスナップショットを監査へ残す。[打刻取消の仕様](spec-improvements-2026-09-11.md)を参照する。
...
料金ルール設定画面は`/extended-care-fees/settings`で提供する。
...
- 請求書発行機能と連携する。詳細は [延長保育料金・請求転送仕様](extended-care-billing-transfer-spec.md) を参照する。
