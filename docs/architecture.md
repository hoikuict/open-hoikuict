...
バックアップ形式2は写真を含む業務DBに加えて施設文例DB・全添付・定期実行設定を保存します。旧形式1の読取りと、登録済みschema契約による追加検査・変換も可能です。秘密値は暗号化した別のrecovery kitで管理し、複製・復元試験の証跡はbackup本体の外へ置きます。稼働中コピーの制限と復元検査は[バックアップ・復元仕様](backup-restore-spec.md)を参照してください。
