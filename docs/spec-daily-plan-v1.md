...
- ステータス: 一部実装（永続化・版管理・実施変更に加え、日案カレンダー・振り返り・確認通知を実装）
- 現況再確認: 2026-09-13
- 比較元:
  - `日案作成機能_設計書_v0.2.md`
  - `DESIGN.md`
...
現在の作成画面は、日付・クラス・年齢・担当者、「ねらい」「内容」と、時刻／活動・子どもの様子・保育士の援助・配慮事項の横4列で構成する。過去の日案例を一組選んで反映し、必要箇所を編集して保存する。新規作成中の行追加・挿入・削除も可能。当初の「4つの本文項目だけ」の案から拡張されている。

過去資料の変換は別リポジトリで行い、匿名化と人による確認を終えた版付きSQLiteだけを本体が読み取り専用で利用する。受け入れ契約は `docs/daily-plan-corpus-contract.md` に定める。

既存日案からのサンプルDB作成と、希望施設への任意導入・更新・停止は[日案サンプルDB仕様](daily-plan-sample-db-spec.md)に定める（計画・未実装）。

以下の版付きテンプレート、構造化した活動ブロック・実績・評価などは拡張設計である。現行の作成フォーム・振り返り本文と、未実装の構造化機能を区別する。
...
- 既存データの `env / children / support` キーは互換テンプレートとして維持する。現在の新規作成は `children / support / considerations` を使う。
...
- 既存データの `env / children / support` は互換テンプレート列として保持する。新規作成の `considerations` と既存の `env` は自動変換しない。
...
文脈API、計画本文の自動保存、項目コメント、専用の提出・差戻し・承認API、構造化した実施記録API、専用印刷URLは後続フェーズで追加する。現時点では文書詳細・編集・状態更新の既存ルートを再利用する。

日案カレンダーと独立した振り返り欄は追加実装済み。

| メソッド | URL | 用途 |
| --- | --- | --- |
| GET | `/plans/daily-plans/` | 月別・クラス別の作成状況と振り返り状況 |
| POST | `/plans/documents/{document_id}/reflection/draft` | 振り返りの下書き保存（HTML応答） |
| POST | `/plans/documents/{document_id}/reflection` | 振り返りの提出 |

`PlanDailyReflectionRow` は1文書につき1件の本文と下書き／提出状態を保存する。計画の不変リビジョンとは別であり、「事実・評価・次への問い」の構造化、振り返りの版履歴、週案への自動集約まで実装したものではない。
...
- 「ねらい」「内容」＋横4列の作成フォーム。作成中の行追加・挿入・削除と自由な時刻表記。
- `/plans/daily-plans/` のカレンダーと作成・振り返り状況。
- `PlanDailyReflectionRow` による振り返り下書き・提出と未提出の案内。基準時刻は `HOIKU_DAILY_REFLECTION_REMINDER_TIME`（既定16:30）。
- `PlanReviewNotificationRow` による管理者への確認依頼・提出者への結果通知。職員ホームの承認待ちと連携。
- `PlanDocumentAction` への判定者名・コメントの保存と、文書詳細への承認・差戻し情報表示。

後続フェーズの機能を一部先行実装しているため、「Phase 2以降がすべて未実装」とは扱わない。下記の初回実装記録はその時点の履歴として残す。
...
- 項目単位のコメント、能力ベース権限の拡張（文書単位の提出一覧・承認結果通知は追加実装済み）
- 事実・評価・次への問いと経緯レコード連携（単一本文の振り返り欄は追加実装済み）
- タグ、AI支援、高度な出力
