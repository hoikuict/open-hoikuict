...
- 文書バージョン: 0.4
- 作成日: 2026年8月23日
- ステータス: 一部実装・設計履歴（現況確認: 2026-09-13）
...
!!! info "現行実装とこの設計書の読み方"
    職員・保護者のローカル認証、メール招待、園の承認、本人によるパスワード変更、園による再設定、明示的な園児リンクは実装済みです。後から追加した初回入力の招待・共通QR・コードのメール送信・24時間有効の保護者コードは、[アカウントガイド](accounts.md)、[家族プロフィール同期](guardian-account-sync.md)、[共通QR登録](parent-public-registration.md)を優先してください。

    本文は初版の設計判断を残しています。第3章の不足点は設計時点の記録であり、現在の不具合・未実装一覧ではありません。MFA等の後続計画と、一括データ移行の未実装部分は引き続き区別します。
...
本仕様はローカル認証の導入前に策定した契約である。現在はモックとlocal_passwordを環境設定で切り替え、保護者ログイン、初期設定、パスワード変更、管理者による再設定を提供している。
...
初版の調査時点では職員認証のみ実装され、保護者はモック認証だった。現在は保護者のローカル認証バックエンド、認証画面、資格情報管理、ライフサイクル処理も実装されている。

### 3.2 β開始前に解消する認可上の問題

初版の調査時点の保護者ポータルには、同じ家庭に属する全園児を自動的に閲覧対象へ広げる処理があった。現在は明示的なリンクだけを用いる。また、家庭の保護者と園児の直積で`ParentChildLink`を再生成する処理がある。この挙動は、再婚家庭、別居、里親、一時的な送迎者等で過剰な閲覧権限を生むため、実データを使うβ開始前に廃止する。
...
現在の入口は `/parent-accounts/{account_id}/authentication` の認証管理画面。管理者が次を確認・操作する。
...
施設管理者が本人確認後に24時間有効な6文字コードを発行し、施設承認済みの方法で本人へ交付する。救済経路でも、管理者による対象保護者と`ParentChildLink`の確認を省略しない。救済コードによる完了を監査し、通常のメール招待と同じパスワードポリシーを適用する。
...
- 停止・再開は管理者が「認証管理」で理由を入力し、対象と影響を確認して確定する。保護者情報編集から状態を変更しない。台帳CSVでも既存アカウントの状態変更を拒否する。
- 通常の一時停止からの再開は、既存パスワードを維持して本人の再ログインを求める。失効済みセッション・通知購読・招待・再設定コードは復活させず、通常再開のメールは送らない（2026-09-23 モック確認後に合意）。
- 再設定を伴う再開は別の操作とする。`parent_resume`コードを登録メールへ送り、本人が`/parent-portal/resume`で設定を完了するまではアカウントと資格情報を停止したままにする。案内取消はコード交換後の入力セッションも失効させる。期限切れは停止中として表示し、案内を再発行できる。
- 再設定必須の資格情報（`must_change_password`）は通常再開の対象にしない。初回未登録、登録確認待ち、メールアドレス削除済みも通常再開で迂回させない。未登録者は認証管理の初回登録・初回設定へ戻る。
- 一覧・編集・園児詳細・認証管理で共通の利用状態を表示する。「台帳は有効・資格情報は停止」の既存データは停止中とし、画面を開くだけで復旧しない。
- 確認後の別操作・二重送信は状態リビジョンとDBの条件付き更新で拒否する。保存失敗時は変更全体をロールバックし、理由を保持して再試行する。操作対象、管理者、理由、日時を監査記録へ保存する。
...
リバースプロキシ配下ではUvicornのproxy headersを有効にし、`FORWARDED_ALLOW_IPS`をプロキシの送信元IPまたはCIDRへ限定する。未設定や`*`を許可すると、全利用者が同じ接続元としてスロットルされたり、逆に外部から偽の`X-Forwarded-For`を注入されたりするため、production起動時に拒否する。
...
- 初期設定・再設定コードはハッシュ保存、24時間、有効1回、再発行時失効となる
