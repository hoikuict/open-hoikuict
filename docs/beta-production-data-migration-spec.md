...
- 作成日: 2026-08-13
- 状況: 一括移行は計画（現況確認: 2026-09-13）
- 関連仕様: [インポート・エクスポート機能仕様](import-export-spec.md)

!!! info "現在の取り込み機能との境界"
    移行元ID、移行マニフェスト、複数CSVの一括検証・一括確定は未実装です。一方、家庭CSVの保護者①②、既存情報の保持、事前検証の差分・競合検査、明示的な園児リンクは実装済みです。現在の手順は[家庭・園児CSV](family-guardian-csv-guide.md)と[入出力仕様](import-export-spec.md)を参照してください。

    本文のリスク調査は初版時点の履歴です。家庭内リンクの直積生成は廃止され、初期管理者は外部認証との対応付けではなくローカル認証CLIで作成します。外部認証を前提とした本文の手順は現行運用へ適用せず、[アカウントガイド](accounts.md)を使ってください。
