...
家族口座情報の編集フォームは `current_user.can_edit` で表示され、更新APIも `require_can_edit` で保護されている。このため、担任、看護師、栄養士、事務職員等のうち、基本ロールが `can_edit` の職員は全員、家族口座情報を編集できる。

また、既存の職員編集画面には基本ロールと「園児台帳管理」権限が混在しており、業務別権限が増えた場合に設定場所が分かりにくくなる。
