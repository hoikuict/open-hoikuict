...
さらに、対象月に降園打刻があるのに日別料金が存在しない出欠は、未計算として転送を禁止する。その際に認定・対応ルール・必要打刻の不足理由を取得して表示する。すでに料金が存在する出欠はこの検査から除外されるため、現在の認定・ルールで全料金を再評価する処理ではない。条件の詳細は[保育認定・区分別料金](care-need-certification-spec.md)を参照する。
