...
- 保護者登録状態サンプル: seedへの追加は計画（認証・登録の機能自体は実装済み）
...
- 保護者アカウント156件
- 外国籍想定家庭9世帯（全家庭の10.7％）
- 同梱CSVの登降園・日次連絡は2026-04-13〜2026-05-15。seedでは実行日基準の期間も動的生成（後述）
...
[開発手順](development.md)で専用DBの `HOIKUICT_DATABASE_URL` を設定してから実行します。CLIを実行するシェルにもDB設定が必要です。アプリの通常起動はシードを実行しません。
...
次の状態配分はseedへの追加計画である。保護者登録申請モデルと認証機能は実装済みだが、この状態配分の自動生成とは区別する。生のtoken、平文パスワード、実際に配送可能なメールはseedへ含めない。
