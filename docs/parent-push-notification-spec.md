...
| HTTP 401 / 403 | `vapid_auth_failed` として終端失敗し、ERRORログと運用警告の対象。他の4xxへ集約しない |
...
現行の保存期間処理は `initialize_application()` によるアプリ起動時にだけ実行されるため、長期間再起動しないプロセスでは次回起動までpurgeされない。Phase 2でproduction運用を開始する前に、ワーカーループで最終実行日を記録し、日付が変わった後に1回実行する日次フックを追加する。
