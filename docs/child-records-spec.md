...
2026-09-14に園児マスタの `sex` を実装。内部値は `male` / `female` / `not_set`、画面は「男・女・未設定」とする。既存園児は未設定とし、未設定でも登録・編集・変更申請できる。園児CSV/Excelの任意列「性別」で入出力する。保護者の性別項目は設けない。保育要録の生成・承認は引き続き後続フェーズとする。
