...
let s={flow:'home',step:0,local:false,lan:false,public:false,lanPending:false,publicPending:false,trialRunning:false,
  v:{name:'',email:'',login:'',password:'',confirm:'',reason:'このPCでのβ版試用',actor:'',approver:'',path:'',port:'8001',
    facility:'',adapter:'',ip:'',subnet:'',ipReserved:false,tls:'domain',hostname:'',localHostname:'hoikuict.home.arpa',dnsToken:'',dnsReady:false,
    smtpHost:'',smtpPort:'587',smtpUser:'',smtpPassword:'',mailFrom:'',testRecipient:'',mailReceived:false,
...
function render(focus=true){
  ...
  if(busy)panel.querySelectorAll('button').forEach(b=>b.disabled=true);
...
async function saveDraft(){if(s.local&&['lan','public'].includes(s.flow)&&!s.lanPending&&!s.publicPending)await api('server/draft',{flow:s.flow,step:s.step,values:s.v});}
async function poll(){
  try{
    if(s.operation==='local'){
      ...
      if(job.state==='complete'){s.applying=false;s.failed=false;s.error='';s.v.dnsToken='';s.v.smtpPassword='';s.v.tunnelToken='';
...
panel.addEventListener('change',e=>{if(e.target.name&&Object.hasOwn(s.v,e.target.name)){change(e.target.name,e.target.type==='checkbox'?e.target.checked:e.target.value);if(e.target.tagName==='SELECT'){if(e.target.name==='adapter'){const a=s.adapters.find(a=>a.id===s.v.adapter);if(a){s.v.ip=a.ip;s.v.subnet=a.subnet;}}render(false);}}});
panel.addEventListener('click',async event=>{
  ...
  if(a==='pause'){confirmDialog('いったん中断しますか？',s.applying?'処理の区切りで中断し、今回の設定を戻します。':'現在の利用状態を維持して中断します。秘密情報は、画面を閉じると再入力が必要です。','中断する',async()=>{if(s.applying){await api(s.operation==='local'?'cancel':'server/cancel',{});}else{await saveDraft();s.paused=true;render();}});return;}
  ...
  try{
    if(a==='start-local'){s.flow='local';s.step=0;if(!s.release)await loadRelease();}
    ...
    else if(a==='back'){if(s.step===5&&s.flow==='lan'){s.flow='home';s.step=0;}else if(s.step)s.step--;else s.flow='home';}
      ...
      const kind=a.slice(6);s.notice='確認しています…';render(false);
  ...
  }catch(e){s.error=e.message;if(e.code==='release_changed'){s.step=2;await loadRelease();}}
...
document.querySelector('#exit').addEventListener('click',()=>confirmDialog('導入アプリを終了しますか？',s.applying?'設定中です。いったん中断してから終了してください。':s.lan||s.lanPending?'園内サーバーは動き続けます。入力中の秘密情報は保存しません。':'このPCの試用アプリを停止して終了します。','終了する',async()=>{if(s.applying)throw new Error('設定処理を中断してから終了してください。');await saveDraft();await api('exit',{});clearTimeout(timer);s.v.password=s.v.confirm=s.v.dnsToken=s.v.smtpPassword=s.v.tunnelToken='';panel.innerHTML='<h2>終了しました</h2><p>この画面を閉じてください。</p>';}));
(async()=>{
  try{
    ...
    if(!server.state&&server.draft?.values){s.v={...s.v,...server.draft.values};if(Object.keys(server.draft.values).length)s.notice='下書きを読み込みました。秘密情報と接続確認は、もう一度入力・実行してください。';}
